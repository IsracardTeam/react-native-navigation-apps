...
class NavigationApps extends Component {
                ...
                'googleMaps': {
                    ...
                    ...Platform.select({
                        ...
                        android: {
                            ...
                            navigateByAddress: ({addressToNavigate, travelMode}) => this.state.navApps.googleMaps.appDeepLinkUriToUse + `dir/?api=1&destination=${addressToNavigate}&travelmode=${travelMode}`,
                            navigateByLatAndLon: ({addressToNavigate, travelMode, lat, lon}) => encodeURI(this.state.navApps.googleMaps.appDeepLinkUriToUse + `search/?api=1&query=${lat},${lon}`),
                            searchLocationByLatAndLon: ({lat, lon}) => encodeURI(this.state.navApps.googleMaps.appDeepLinkUriToUse + `search/?api=1&query=${lat},${lon}`)
                        }
                    }),
                    ...googleMaps,
    ...
    renderNavigationAppsAsActionSheet = () => {
        ...
        const actionSheetOptionsDisplayName = () => {
          const {actionSheetBtnCloseTitle} = this.props;
          const {navApps} = this.state;
          const actionSheetArray = Object.keys(navApps).map((navApp, key) => {

            const navAppItem = navApps[navApp];
            return navAppItem.displayName
        ...
        return (
            <React.Fragment>
                ...
                <ActionSheet
                    ...
                    options={actionSheetOptionsDisplayName()}
                    cancelButtonIndex={actionSheetOptionsDisplayName().length - 1}
                    destructiveButtonIndex={actionSheetOptionsDisplayName().length - 1}
                    onPress={async (index) => {

                        if (index !== actionSheetOptionsTitle().length - 1) {
                            await this.handleNavApp(actionSheetOptionsTitle()[index])
                        }
